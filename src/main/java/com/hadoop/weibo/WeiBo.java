...
public class WeiBo {
	...
	/**
	 * 表名：ns_weibo:content 列族名：info 列名：content rowkey:用户id_时间戳
	 * value:微博内容（文字内容，图片URL，视频URL，语音URL） versions:1
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * 表名：ns_weibo:relation 列族名：attends，fans 列名：用户id value：用户id
	 * rowkey：当前操作人的用户id versions:1
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * 发布微博 a、向微博内容表中添加刚发布的内容，多了一个微博rowkey b、向发布微博人的粉丝的收件箱表中，添加该微博rowkey
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * a、在用户关系表中，对当前主动操作的用户id进行添加关注的操作 b、在用户关系表中，对被关注的人的用户id，添加粉丝操作
	...
	/**
	 * a、在用户关系表中，删除你要取关的那个人的用户id b、在用户关系表中，删除被你取关的那个人的粉丝中的当前操作用户id
	 * c、删除微博收件箱表中你取关的人所发布的微博的rowkey 取关操作
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * 查看微博内容 a、从微博收件箱中获取所有关注的人发布的微博的微博rowkey b、根据得到的微博rowkey，去微博内容表中得到数据
